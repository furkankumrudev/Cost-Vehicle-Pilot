...
export function PriceTrendChart({ filters }: { filters: Filters }) {
  ...
  return <section className="chart-card section-block"><div className="section-heading"><div><span className="eyebrow">FİYAT TRENDİ</span><h2>İkinci El Araç Fiyat Trendi</h2><p>{data?.label ?? "İlan tarihine göre fiyat görünümü"}</p></div><div className="segmented"><button className={stat === "median" ? "selected" : ""} onClick={() => setStat("median")}>Medyan</button><button className={stat === "average" ? "selected" : ""} onClick={() => setStat("average")}>Ortalama</button></div></div>
    ...
    {loading ? <LoadingSkeleton rows={5} /> : error ? <ErrorState detail={error} /> : !data?.available ? <EmptyState title="Geçmiş trend oluşmadı" detail={data?.message} /> : <div className="chart-wrap"><ResponsiveContainer width="100%" height="100%"><LineChart data={data.points} margin={{ top: 12, right: 14, left: 16, bottom: 6 }}><CartesianGrid stroke="#e7edf5" strokeDasharray="3 4" /><XAxis dataKey="date" tickFormatter={shortDate} tickLine={false} axisLine={false} minTickGap={30} /><YAxis tickFormatter={(value) => `${number(value / 1_000_000)} Mn`} tickLine={false} axisLine={false} width={52} /><Tooltip formatter={(value, name) => [money(Number(value)), name]} labelFormatter={(value) => shortDate(String(value))} /><Legend content={<ListingChartLegend showClean={data.clean_available} />} /><Line type="monotone" dataKey={priceKey} name="Tüm ilanlar" stroke="#2563eb" strokeWidth={3} dot={false} activeDot={{ r: 5 }} />{data.clean_available && <Line type="monotone" dataKey={cleanPriceKey} name="Temiz araç ilanları" stroke="#16875b" strokeWidth={3} dot={false} activeDot={{ r: 5 }} connectNulls />}</LineChart></ResponsiveContainer></div>}
  </section>;
}
