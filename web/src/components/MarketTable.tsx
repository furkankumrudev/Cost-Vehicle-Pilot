...
export function MarketTable({ filters }: { filters: Filters }) {
  ...
  return <section className="section-block table-section"><div className="section-heading"><div><span className="eyebrow">KARŞILAŞTIRMA</span><h2>Marka karşılaştırma tablosu</h2><p>Fiyat değişimleri için gerçek tarihsel snapshot birikimi gerekir.</p></div><div className="table-controls"><label className="search-input"><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Marka ara" /></label></div></div>
