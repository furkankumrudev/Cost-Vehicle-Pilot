@echo off
cd /d "%~dp0.."
echo Pulling general listings from the last 24 hours...
...
".venv\Scripts\python.exe" -m src.ingestion.recent_listing_scraper --days 1 --filter-segments general --max-pages 0 --page-size 50 --price-bands "%PRICE_BANDS%" --delay-min 3 --delay-max 6 --manual-wait-seconds 180 --max-old-pages 3 --max-stale-pages 0 --max-repeated-pages 3 --stop-on-access --checkpoint-path data\runtime\daily_recent_checkpoint.json %*
