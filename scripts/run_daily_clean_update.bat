...
".venv\Scripts\python.exe" -m src.ingestion.recent_listing_scraper --days 1 --filter-segments clean --max-pages 0 --page-size 50 --price-bands "%PRICE_BANDS%" --delay-min 3 --delay-max 6 --manual-wait-seconds 180 --max-old-pages 3 --max-stale-pages 0 --max-repeated-pages 3 --stop-on-access --checkpoint-path data\runtime\daily_clean_recent_checkpoint.json %*
